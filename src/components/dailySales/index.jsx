...
const SalesTable = () => {
  ...
  const [filter, setFilter] = useState("all"); // Estado para filtrar por método de pago
  ...
  // Función para filtrar las ventas según el método de pago seleccionado y la fecha seleccionada
  const filterSalesByPaymentMethodAndDate = (method, date) => {
    let filteredSales = [...sales];

    if (date) {
      filteredSales = filterSalesByDate(filteredSales, date);
    }
  ...
  const renderFilteredTable = () => {
    ...
    if (selectedDate || filter !== "all") {
      filteredSales = filterSalesByPaymentMethodAndDate(filter, selectedDate);
    }
              ...
              {filteredSales.map((sale) => (
                <tr key={sale.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <ul>
                      {sale.products.map((product) => (
                        <li key={product.id}>${products[product.name]}</li>
                      ))}
                    </ul>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
  ...
  return (
    <div className="flex flex-col items-center mt-8">
      <div className="flex items-center mb-4">
        <div className="mr-8">
          <label htmlFor="dateInput" className="block mb-1">Seleccionar fecha:</label>
          <DatePicker
            id="dateInput"
